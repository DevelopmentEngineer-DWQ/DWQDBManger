...
@implementation DWQDBModel
#pragma mark - override method
+ (void)initialize
{
    if (self != [DWQDBModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    DWQDBTool *dwqDB = [DWQDBTool shareInstance];
...
+ (BOOL)createTable
{
    ...
    DWQDBTool *dwqDB = [DWQDBTool shareInstance];
    [dwqDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
{
    //判断是否是JKBaseModel的子类
    for (DWQDBModel *model in array) {
        if (![model isKindOfClass:[DWQDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [dwqDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            DWQDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
